...
import net.neoforged.gradle.mcp.extensions.ChannelProvidersExtension;
import net.neoforged.gradle.mcp.MCPPlugin;
import net.neoforged.gradle.mcp.MCPRepo;
import net.neoforged.gradle.mcp.McpExtension;
import net.neoforged.gradle.mcp.runtime.McpRuntimeDefinition;
import net.neoforged.gradle.mcp.runtime.extensions.McpRuntimeExtension;
import net.neoforged.gradle.mcp.runtime.tasks.AccessTransformer;
import net.neoforged.gradle.mcp.runtime.tasks.SideAnnotationStripper;
import net.neoforged.gradle.mcp.tasks.DownloadMCPConfig;
import net.neoforged.gradle.mcp.tasks.DownloadMCPMappings;
import net.neoforged.gradle.mcp.tasks.GenerateSRG;
import net.neoforged.gradle.mcp.tasks.WriteEntriesTask;
import net.neoforged.gradle.mcp.runs.RunGenerationUtils;
import net.neoforged.gradle.patcher.tasks.*;
...
public class PatcherPlugin implements Plugin<Project> {
    ...
    public void apply(@Nonnull Project project) {
        ...
        final TaskProvider<MojangLicenseTask.Hide> hideLicense = tasks.register(MojangLicenseTask.HIDE_LICENSE, MojangLicenseTask.Hide.class);
        final TaskProvider<MojangLicenseTask.Show> showLicense = tasks.register(MojangLicenseTask.SHOW_LICENSE, MojangLicenseTask.Show.class);
        final TaskProvider<MojangLicenseTask.Display> displayLicense = tasks.register(MojangLicenseTask.EMIT_LICENSE, MojangLicenseTask.Display.class);
        ...
        project.afterEvaluate(p -> {
...

            {
                ...
                Provider<File> clientJar = version.zip(classifier, (ver, cls) -> {
                    final ArtifactDownloaderExtension downloader = project.getExtensions().getByType(ArtifactDownloaderExtension.class);
                    File ret = downloader.generate("net.minecraft:client:" + ver + cls, true).getOrNull();
                ...
                Provider<File> serverJar = version.zip(classifier, (ver, cls) -> {
                    final ArtifactDownloaderExtension downloader = project.getExtensions().getByType(ArtifactDownloaderExtension.class);
                    File ret = downloader.generate("net.minecraft:server:" + ver + cls, true).getOrNull();
                    if (ret == null || !ret.exists())
                ...
                Provider<File> joinedJar = mcpConfigVersion.zip(classifier, (ver, cls) -> {
                    final ArtifactDownloaderExtension downloader = project.getExtensions().getByType(ArtifactDownloaderExtension.class);
                    File ret = downloader.generate("net.minecraft:joined:" + ver + cls, true).getOrNull();
                    if (ret == null || !ret.exists())
