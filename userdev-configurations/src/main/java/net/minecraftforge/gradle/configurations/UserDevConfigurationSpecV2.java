...
package net.minecraftforge.gradle.configurations;
...
public class UserDevConfigurationSpecV2 extends UserDevConfigurationSpecV1 {
    ...
    public static UserDevConfigurationSpecV2 get(final File file) {
        try {
            return get(new FileInputStream(file));
        } catch (FileNotFoundException e) {
            throw new RuntimeException(String.format("Failed to read the userdev configuration from: %s", file.getAbsolutePath()), e);
        }
    }

    public static UserDevConfigurationSpecV2 get(final InputStream stream) {
        return GSON.fromJson(new InputStreamReader(stream), UserDevConfigurationSpecV2.class);
    }

    public static UserDevConfigurationSpecV2 get(final byte[] data) {
