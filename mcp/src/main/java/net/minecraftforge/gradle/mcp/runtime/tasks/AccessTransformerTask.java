...
@CacheableTask
public abstract class AccessTransformerTask extends ExecutingMcpRuntimeTask {

    public AccessTransformerTask() {
        super();
                ...
                getInputFile().flatMap(inputFile ->
                        getOutputFile().map(outputFile -> {
                                    ...
                                    final Provider<File> additionalTransformersFileProvider = getAdditionalTransformers()
                                            .flatMap(additionalTransformers -> ensureFileWorkspaceReady(getFileInOutputDirectory(getProjectFileName("transformers.cfg")))
