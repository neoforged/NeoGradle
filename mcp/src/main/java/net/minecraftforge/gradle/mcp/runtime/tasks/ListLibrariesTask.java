...
@CacheableTask
public abstract class ListLibrariesTask extends McpRuntimeTask {
    private static final Attributes.Name FORMAT = new Attributes.Name("Bundler-Format");

    public ListLibrariesTask() {
        super();
    ...
    public void run() throws Exception {
        ...
        try (FileSystem bundleFs = getServerBundleFile().isPresent() ? null : FileSystems.newFileSystem(getServerBundleFile().get().getAsFile().toPath(), (Map<String, ?>) null)) {
            ...
            // Write the list
            PrintWriter writer = new PrintWriter(new OutputStreamWriter(new FileOutputStream(output.get()), StandardCharsets.UTF_8));
