...
public final class RunGenerationUtils {
    ...
    public static void createRunConfigTasks(final McpMinecraftExtension extension, final TaskProvider<ExtractNatives> extractNatives, final TaskProvider<?>... setupTasks) {
        ...
        extension.getRunConfigurations().forEach(RunConfiguration::mergeParents);
        ...
        extension.getProject().getGradle().projectsEvaluated(gradle -> {
            ...
            extension.getRunConfigurations().forEach(RunConfiguration::mergeChildren);
            extension.getRunConfigurations().forEach(run -> RunConfigGenerator.createRunTask(run, extension.getProject(), prepareRuns, additionalClientArgs));
