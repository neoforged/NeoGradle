...
final class CacheableMinecraftVersion implements Comparable<CacheableMinecraftVersion>, Serializable {
    ...
    private static CacheableMinecraftVersion get(String version) {
        ...
        } else if (first == 'a' as char || first == 'b'  as char || first == 'c' as char) {
            String clean = version.substring(1).replace('_', '.');
            String nearest = clean;
            String rev = null;
            char end = clean.charAt(clean.length() - 1);
            if (end < ('0' as char) || end > ('9' as char)) {
                rev = Character.toString(end);
                nearest = clean.substring(0, clean.length() - 1);
            }

            return new CacheableMinecraftVersion(first == 'b' as char ? Type.BETA : Type.ALPHA, version, -1, -1, 0, rev, splitDots(nearest));
        } else if (version.length() == 6 && version.charAt(2) == 'w' as char) {
    ...
    @Override
    int compareTo(@Nullable CacheableMinecraftVersion o) {
        if (o == null)
            return 1;
